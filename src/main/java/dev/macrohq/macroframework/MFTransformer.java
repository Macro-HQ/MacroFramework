...
public class MFTransformer implements IClassTransformer {
    @Override
        ...
        if (name.equals("org.lwjgl.nanovg.NanoVGGLConfig")) {
            ...
            for (MethodNode method : node.methods) {
                if (method.name.equals("configGL")) {
                    ...
                    list.add(new TypeInsnNode(Opcodes.NEW, "dev/macrohq/macroframework/Lwjgl2FunctionProvider"));
                    list.add(new InsnNode(Opcodes.DUP));
                    list.add(new MethodInsnNode(
                            Opcodes.INVOKESPECIAL,
                            "dev/macrohq/macroframework/Lwjgl2FunctionProvider",
                            "<init>",
                            "()V",
                            false
                    ));
